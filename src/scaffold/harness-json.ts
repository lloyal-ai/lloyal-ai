 ...
 * any local value the key cannot take. A version-1 file — written before alpha.10 — is refused by name, as rig
...
export function readLocalModel(projectDir: string): Record<string, Bag> | null {
  ...
  if (parsed.version === 1) throw new Error('harness.json is version 1, written before alpha.10 — delete it and relaunch.');
