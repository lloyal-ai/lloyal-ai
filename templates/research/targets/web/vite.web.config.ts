...
export default defineConfig({
  ...
  server: {
    port: 5173,
    // Both planes reach the browser through THIS origin in dev: the page is on
    // :5173, the host on :8787. Proxying the content plane keeps its requests
    // same-origin, so no CORS preflight is involved and the host needs no
    // `allowedOrigin` for local work. `/v1/media` carries uploads and
    // representations; `/v1/content` answers existence by digest.
    proxy: {
