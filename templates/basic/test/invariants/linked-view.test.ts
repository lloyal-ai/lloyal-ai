...
test("the web config pre-bundles every platform entry the page loads a value from", () => {
  ...
  // The walk really did reach the view. basic imports ui by SUBPATH only — the root re-exports the React
  // renderer, and `src/ui/state.ts` is folded by the Electron main process too — so the fold is the check.
  assert.ok(reached.has("@lloyal-labs/ui/fold"), `the walk reaches the view: ${[...reached].join(", ")}`);
