...
describe('modelSelection', () => {
  ...
  it('a version-1 overlay is read at the blocks its keys now live in; an unknown version is not read', () => {
    yml('model:\n  llm:\n    id: qwen3.5-4b\n');
    json({ version: 1, model: { rerankerId: 'r1', reranker: '/r1.gguf' } });
    expect(modelSelection(dir, 'reranker')).toEqual({ present: true, spec: { path: '/r1.gguf' } });
